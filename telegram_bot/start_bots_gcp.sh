...
# Use venv Python if available (required on Debian 12+ where system pip is locked)
PYTHON="python3"
...
nohup $PYTHON forex_scalper.py >> logs/forex_scalper.log 2>&1 &
echo "  [OK] forex_scalper.py — PID $!"

# Nifty scalper (Indian market hours only)
nohup $PYTHON nifty_scalper.py >> logs/nifty_scalper.log 2>&1 &
