"""
BTC Trading Signal Bot — BTCUSD (Windows Native)
...
- Logs to C:\TradingBots\logs\btc_bot.log
...
def send_daily_report():
    ...
    lines = [
        f"[BTC BOT] 📊 <b>Daily Signal Report — {today}</b>",
        "━━━━━━━━━━━━━━━━━━━━━━",
...
def fetch_ohlcv():
    ...
    if df is None or len(df) < SLOW_EMA + 10:
        ...
        return None
    _cache["1h"] = (now, df)
    log.info("Fetched %d 1H bars for BTCUSD", len(df))
    return df

def get_daily_trend():
    """Get daily trend (used for secondary confirmation)."""
    now = time.time()
...
def main():
    ...
    _load_seen_bars()
    log.info("BTC Bot started | ema=%d/%d  rsi=%d  cache=%ds  poll=%ds",
